...
## Cygwin pythonはPython2なので、AnacondaのPython3が先に見つかるようにする
...
plotly::ggplotlyを使うと、インタラクティブに拡大縮小したり、線にカーソルを置いて各年の人口を表示できたりする。ただし、出力されるHTMLファイルが数Mbytes増えるのと、PDF化できなくなる。
...
```{r are_equal_calc}
assertthat::are_equal(x = df_wards_calc$per_household,
                      y = df_wards_calc$per_household_calc, tol = 0.01)
assertthat::are_equal(x = df_wards_calc$density,
...
```{r near_calc}
are_near_value <- dplyr::near(x = df_wards_calc$per_household,
                              y = df_wards_calc$per_household_calc, tol = 0.01)
are_near_value
all(are_near_value)
```
