...
Python の `range` てきな連番を作ろう。要素数n個のベクトルvに対して 1, 2, ..., n という連番の整数は、 `seq_len(n)` で作る。 `seq_len(NROW(c(10, 20, 30)))` は要素が3個なので `c(1, 2, 3)` になる。 `seq_len(NROW(c()))` は空ベクトルが返る。
...
tibble に比べれば覚えることが少ないので、最低限のことだけ説明する。
...
df_wards_calc <- df_wards %>%
  dplyr::mutate(per_household_calc = population / household,
