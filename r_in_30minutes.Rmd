...
数値には整数と浮動小数がある。ここでは型変換やオーバーフローなどの細かい説明は省略して、Pythonと同様に整数か浮動小数かを意識して計算すればそうなる、ということで話を進める。Python組み込みのintは桁数に上限が無い(任意精度整数)。Rの整数は、NumPyのint32のようなものである。
...
Python の `A and B` は、A-ならば-Bという短絡評価である。短絡評価とは、Aが成立しなければBは評価しないということである。同様に `A or B` は、A-さもなくば-Bという短絡評価である(Aが成立すればBは評価しない)。And を「かつ」、 or を「または」と訳すのは論理演算であって、短絡評価では「ならば」「さもなくば」と訳すのが正しい。

ある名前のファイルが空でないかどうか調べるには、その名前のファイルが存在することをまず確かめなければならない。ファイルがあればそのサイズを取得する(なければ取得しない)のが短絡評価の使い方である。

```{python short_circuit_python, result="asis", error=TRUE, class.source="python-code", class.output="python-output"}
import os
def non_empty_file(filename):
  return os.path.isfile(filename) and (not (os.path.getsize(filename) == 0))

non_empty_file("r_in_30minutes.Rmd")
non_empty_file("_file_does_not_exist")
```

Rの短絡評価は、and の代わりに `&&` 、or の代わりに `||` という演算子を用いる。ついでに not は `!` である。これらはC++と同じである。

```{r short_circuit_r, class.source="r-code", class.output="r-output"}
non_empty_file <- function(filename) {
  file.exists(filename) && !(file.info(filename)$size == 0)
}
...
`&` や `|` は `+` と同様、ベクトルの各要素に対して and や or を取る。うっかり間違えて `&&` や `||` と書いた場合、ベクトルの最初の要素についてだけ結果を返し、ベクトルの要素が複数個あってもエラーにはならない。

```{r element_wise_and, error=TRUE, class.source="r-code", class.output="r-output"}
c(FALSE, TRUE, TRUE) & c(NA, TRUE, NA)
c(FALSE, TRUE, TRUE) && c(NA, TRUE, NA)
```
...
```{r set_operations, class.source="r-code", class.output="r-output"}
union(1:4, 3:7)
intersect(1:4, 3:7)
setdiff(1:4, 3:7)
setdiff(3:7, 1:4)
...
```{r in_operator, class.source="r-code", class.output="r-output"}
1:7 %in% 3:4
3:4 %in% 1:7
```
...
```{python default_python_suspicious, result="asis", class.source="python-code", class.output="python-output"}
add()
add()
```
...
invisible(assertthat::assert_that(all(are_near_values)))
```
...
Pythonでコードを試しながら書く場合、Jupyter Notebook を使っている方が多いだろう。Jupyter Notebook とRという組み合わせも可能である。もう一つの選択肢は、 R Markdown である。まさにこの文書の元は R Markdown で、それを直接HTMLに変換したものか、 Markdown に変換したもののHTML表示を見ているはずである。 Assertion を書くと、 R Markdown 文書に埋め込んだコードが間違っているときに、HTMLに変換できなくてエラーになるので分かる。
...
cloc パッケージで、ファイルの論理行、コメント行、空行を数えることができる。Rだけでなくさまざまなプログラミング言語に対応している。Perl が必要なので、Windows の場合は perl.exe がある場所にPATHを通しておく。ファイル名だけでなくURLを指定することもできる。[A Shiny app developed with the golem package](https://github.com/zettsu-t/nbinomPlot) にあるファイルの行数を測ろう(以下の行数は測定当時)。
...
- RStudioを用いたより進んだ話題は、「Rが生産性を高める 〜データ分析ワークフロー効率化の実践」
...
|Rが生産性を高める 〜データ分析ワークフロー効率化の実践|igjit，atusy，hanaori 著|2022|技術評論社|
...
本 R Markdown からHTML文書やPDF文書を生成した場合、 [横浜市のオープンデータ](https://www.city.yokohama.lg.jp/city-info/yokohamashi/tokei-chosa/portal/opendata/) を編集・加工したものが埋め込まれます。これらは [利用条件等](https://www.city.yokohama.lg.jp/city-info/yokohamashi/tokei-chosa/portal/opendata/opendata.html) に記載の、 [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.ja) に基づいています。
